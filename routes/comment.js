			...
			User.findById({
				...
				.then((userResult) => {
					if (userResult) {
						...
						Comment.updateOne(
							{ _id: comment_id },
							{ other_comments: commentResult.other_comments, is_handle: 2 }
						)
							.then((result) => {
...
exports.changeComment = (req, res) => {
	...
	Comment.updateOne({ _id: id }, { state: Number(state), is_handle: 1 })
		.then((result) => {
	...
	Comment.findById({ _id: id })
		.then((commentResult) => {
			...
			if (commentResult.other_comments.length) {
				...
				Comment.updateOne({ _id: id }, { other_comments: commentResult.other_comments, is_handle: 1 })
					.then((result) => {
						responseClient(res, 200, 0, '操作成功', result);
					})
					.catch((err1) => {
				...
				responseClient(res, 200, 1, '第三方评论不存在', commentResult);
			}
		})
		.catch((err) => {
			console.error(err);
